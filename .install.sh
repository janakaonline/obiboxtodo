...
if [ -f "$FILE2" ]; then
    ...
    if [ -f "$FILE" ]; then
        ...
        apt-get -y install openssl php php-common php-cli php-xml php-zip php-mysql php-gd php-imagick php-tidy php-xmlrpc php-mbstring php-curl php-json php-bcmath > /dev/null 2>&1

        echo "30% completed - Installing dependencies (this can take a while)..."

        composer create-project laravel/laravel tasks-management > /dev/null 2>&1
        composer global require hirak/prestissimo > /dev/null 2>&1 
        cd tasks-management && composer install > /dev/null 2>&1
